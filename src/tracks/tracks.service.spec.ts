...
const black_album_tracks = data2list(data.tracks)
    .filter(track => track_titles.includes(track.title));
...
describe('TracksService', () => {
    ...
    describe('create tracks', () => {
        ...
        create_tracks.forEach(track => {
            it(`should return track ${track.title} infos`, async () => {
                ...
                const feats = await Promise.all(track.feats.map((feat: { email: string; }) =>
                ...
                expect(result.feats).toStrictEqual(feats.map((feat) =>
                ({
                    _id: feat._id,
    ...
    describe('When ask one track by id', () => {
        it('should return one track', async () => {
            const track = await tracksService.findByTitle(tracks.change_clothes.title);
    ...
    describe('When ask one track by title', () => {
        ...
        it('should return one track', async () => {
            const result = await tracksService.findByTitle(tracks.change_clothes.title);
