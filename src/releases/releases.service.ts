...
export class ReleasesService {
    ...
    async create(
        ...
        const feats: UserDocument[] = await this.usersService.findManyUsersByUsernames(createRelease.feats.map((feat) => feat.username));
        ...
        try {
            const tracks: ITrackResponse[] = await this.tracksService.createManyTracks(createRelease.tracks.map(
                (track) => 
                    ({
                        ...track,
                        author,
    ...
    private buildReleaseInfo(release: Release, feats: UserDocument[]): IReleaseResponse {
        return {
            title: release.title,
    ...
    private async simpleToUser(simpleUser: AuthorDto) {
