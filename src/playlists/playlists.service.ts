...
import { PlaylistUpdateTaskAction, UpdatePlaylistDto } from './dto/update-playlist.dto';
...
export class PlaylistsService {
    ...
    async updatePlaylist(
        ...
        const trackToUpdate = await this.tracksService.findTrackById(updatePlaylistDto.trackId);
        if (!trackToUpdate) {
            throw new NotFoundException(`Track with ID "${trackToUpdate}" not found.`);
        }
        ...
        switch (updatePlaylistDto.action) {
            ...
            case PlaylistUpdateTaskAction.Remove:
                newTracks = playlistTracks.filter((track) => track._id.toString() !== trackId);
                break;
            default:
    ...
    private async isUserTheOwnerOfPlaylist(playlist: PlaylistDocument, owner: UserDocument) {
